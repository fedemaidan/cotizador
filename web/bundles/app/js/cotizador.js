...
function generarPDF() {
	...
	$.ajax({
		...
		if (data.result) {
			...
			datos = generarPresupuesto();

			//ajax
			var url = $("#generarPDF").data("url");
			datosGenerales = datosGeneralesDelPresupuesto(true);

			var url = url + "?datos=" + datos + "&datosGenerales=" + datosGenerales;
...
function generarPresupuesto() {
	var urlPresupuesto = Routing.generate('cotizar', {captcha: $('#captcha')[0].value}, false);
